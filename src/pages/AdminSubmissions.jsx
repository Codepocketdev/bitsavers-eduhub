...
function AssessmentSubmissions({ assessment, cohort }) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, marginBottom: 14, overflow: 'hidden' }}>
      <div onClick={() => setOpen(p => !p)} style={{ padding: '16px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${C.border}`, padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            <button onClick={sync} disabled={fetching} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: C.dim, border: `1px solid ${C.border}`, color: C.accent, padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
          ...
          {!loadingMembers && pending.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              {pending.map(m => (
                <div key={m.npub} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.1)', borderRadius: 8, marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: C.text }}>{m.name || 'Anonymous'}</div>
                    <div style={{ fontSize: 10, color: C.muted, fontFamily: 'monospace' }}>{m.npub?.slice(0, 20)}…</div>
                  </div>
                </div>
              ))}
            </div>
